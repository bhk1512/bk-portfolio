...
function AxisBreak() {
  return (
    <svg
      ...
      fill="none"
      aria-hidden="true"
      className="absolute left-0 top-0 h-3.5 w-10 -translate-y-1/2 text-hairline-strong"
    >
...
function HorizontalRail({ active, onSelect }: RailProps) {
  ...
  return (
    <div role="group" aria-label="Career timeline" className="flex items-stretch min-w-[560px]">
      ...
      {rest.map((node, restIndex) => {
        ...
        return (
          <div
            key={node.id}
            className="relative"
            style={{ flexGrow: node.weight, flexBasis: 0, minWidth: 64 }}
          >
            <div className="absolute inset-x-0 top-0 h-px bg-hairline-strong" />
            <button
              ref={(element) => {
...
export default function Timeline() {
  ...
  return (
    <div>
      {/* Scrolls horizontally below the rail's min-width instead of
          switching to a vertical layout, so the timeline reads the same
          way at every viewport. */}
      <div className="overflow-x-auto -mx-6 px-6 sm:-mx-10 sm:px-10 lg:-mx-16 lg:px-16">
